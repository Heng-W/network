...
public:
    ...
    void onConnect()
    {
        if (numConnected_.fetch_add(1) == sessionCount_ - 1)
        {
            LOG(WARN) << "all connected";
        }
    }
    ...
    void onDisconnect(const TcpConnectionPtr& conn)
    {
        if (numConnected_.fetch_sub(1) == 1)
        {
            ...
            LOG(WARN) << totalBytesRead << " total bytes read";
            LOG(WARN) << totalMessagesRead << " total messages read";
            LOG(WARN) << static_cast<double>(totalBytesRead) / static_cast<double>(totalMessagesRead)
                      << " average message size";
            LOG(WARN) << static_cast<double>(totalBytesRead) / (timeout_ / 1000 * 1024 * 1024)
                      << " MiB/s throughput";
