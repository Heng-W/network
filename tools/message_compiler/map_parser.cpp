...
class MapParser: public TypeParser
{
public:
    MapParser()
    {
    ...
    vector<string> includeList() const override { return {"<map>", "<numeric>"}; }

    string realType() const override
    {
        return "std::map<" + innerTypes_[0]->realType() + ", " + innerTypes_[1]->realType() + ">";
    }
...
REGISTER_TYPE_PARSER(map, MapParser);
